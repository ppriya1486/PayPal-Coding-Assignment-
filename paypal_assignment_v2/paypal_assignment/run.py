...
def txn_search():
    if request.method == "POST":
        ...
        with open("TxnList.json", "r") as f:
            txn_list = json.load(f)
        txn_list_display = []
        for x in txn_list:
            if x["Payer ID"].lower() == payer_id.lower():
                txn_list_display.append(x)
        if txn_list_display:
            return Response(json.dumps(txn_list_display, indent=2), mimetype='application/json')
        else:
            return Response(json.dumps({"message": "No Transaction Found for this Payer ID"}, indent=2), mimetype='application/json')
    return render_template('txn_list.html')
